...
public class LongestSubArraySumKWithNegatives {
    public static int getLongSubArrraySize(int arr[], int k){
        ...
        for(int i=0;i<arrLength;i++){
            prefixSumTillIndex+=arr[i]; // prefix sum till current index
            if(prefixSumTillIndex==k) // if prefixsum is equal to k
                longestSubArraySize = Math.max(longestSubArraySize,i+1); // max of current index and current longest
            long rem = prefixSumTillIndex - k;
            if(indexSumPair.containsKey(rem)) // check if somewhere there is sum as prefixSum till now - given k, that shows there is sum k in between
            {
                longestSubArraySize = Math.max(longestSubArraySize,i-indexSumPair.get(rem)); // max of current longest and current index - index of sum as (prefixsum - k)
            }
            if(!indexSumPair.containsKey(prefixSumTillIndex)) // if the prefix sum is already i map do not update the index
