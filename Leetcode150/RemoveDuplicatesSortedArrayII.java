...
//        int nums[] = {1,2,3};
    ...
    public static int removeDuplicates(int[] nums){
        ...
        if(nums.length<=2)
            return nums.length;
        int j=2;// since first element will always be unique so we have to swap starting from 2nd element only
        for(int i=2;i<nums.length;i++){
            if(nums[i]!=nums[j-2]) {// if previous is not equal to current
